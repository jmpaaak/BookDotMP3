...
int main (int argc, char **argv)
{
	...
	for(coor_x = 0; coor_x < screen_width - rows; coor_x++)
        {
            *ptr++  =   0x333333;
        }


	for(coor_x = screen_width-rows; coor_x < screen_width; coor_x++)
        {
            *ptr++  =   0x777777;
        }
    }
    ...
    for(coor_y = 0; coor_y < cols; coor_y++) {
	if(cols%2 != 0) {
       	   ptr = (unsigned long*)pfbmap + screen_width*(coor_y+screen_height/2-cols/2-1) + screen_width-rows;
	}
	else {
           ptr = (unsigned long*)pfbmap + screen_width*(coor_y+screen_height/2-cols/2) + screen_width-rows;
        }
	for (coor_x = rows-1; coor_x >= 0; coor_x--) {
            *ptr++  =   bmpdata[coor_y + coor_x*cols];
        }
    }
