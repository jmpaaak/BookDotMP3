...
void capture_page(char* first_page_name, char* second_page_name)
{
	...
	FILE *first_page_fp = fopen(first_page_name, "wb");
	if(first_page_fp == NULL) {
		printf("first_page_fp open error!!\n");
	}

	FILE *second_page_fp = fopen(second_page_name, "wb");
	if(second_page_fp == NULL) {
		printf("second_page_fp open error!!\n");
	}
	...
	while(1)
	{
		...
		if(wait_count>=1) {
			makeBitmap(first_page_fp, second_page_fp, \
	...
	fb_err:
	close(first_page_fp);
	close(second_page_fp);

	return;
}

void makeBitmap(FILE *first_page_fp, FILE *second_page_fp, \
		unsigned char *videoFrame, int videoWidth, int videoHeight)
{ 
	BITMAPFILEHEADER bmpHeader;
	BITMAPINFOHEADER bmpInfoHeader;

	unsigned char frameBuffer_1[videoHeight][(videoWidth/2)*3];
	unsigned char frameBuffer_2[videoHeight][(videoWidth/2)*3];
	int x,y;
	unsigned short *videptrTemp;
	...
	// set video pointer to the origin of bitmap frame
	origin = videoptr; 
	// make bitmap
	for (x=0;x<videoHeight;x++)
	{
		for(y=0;y<videoWidth/2;y++) {
			videptrTemp = origin +videoWidth*(videoHeight-1-x) \
				      +(videoWidth/2-1-y);

			frameBuffer_1[x][y*3 + 2] = \
			(unsigned char)((*videptrTemp & 0xF800)>>8);
			frameBuffer_1[x][y*3 + 1] = \
			(unsigned char)((*videptrTemp & 0x07E0)>>5);
			frameBuffer_1[x][y*3 + 0] = \
			(unsigned char)((*videptrTemp & 0x001F)<<3);
		}
	}

	// set magic number
	fputc('B', first_page_fp);
	fputc('M', first_page_fp);

	// set bitmap header
	memset(&bmpHeader,0,sizeof(bmpHeader));
	bmpHeader.bfSize = (videoWidth/2)*videoHeight*3 + 54;
	...
	bmpInfoHeader.biSizeImage = 320*480*3;

	fwrite((void*)&bmpHeader, sizeof(bmpHeader), 1, first_page_fp);
	fwrite((void*)&bmpInfoHeader, sizeof(bmpInfoHeader), 1, first_page_fp);
	fwrite(frameBuffer_1, bmpInfoHeader.biSizeImage, 1, first_page_fp);
	...
	for (x=0;x<videoHeight;x++)
	{
		for(y=0;y<videoWidth/2;y++) {
			...
			frameBuffer_2[x][y*3 + 2] = \
			(unsigned char)((*videptrTemp & 0xF800)>>8);
			frameBuffer_2[x][y*3 + 1] = \
			(unsigned char)((*videptrTemp & 0x07E0)>>5);
			frameBuffer_2[x][y*3 + 0] = \
			(unsigned char)((*videptrTemp & 0x001F)<<3);
		}
	}

	// make second page bmp 

	// set magic number
	fputc('B', second_page_fp);
	fputc('M', second_page_fp);
	...
	fwrite((void*)&bmpHeader, sizeof(bmpHeader), 1, second_page_fp);
	fwrite((void*)&bmpInfoHeader, sizeof(bmpInfoHeader), 1, second_page_fp);
	fwrite(frameBuffer_2, bmpInfoHeader.biSizeImage, 1, second_page_fp);
